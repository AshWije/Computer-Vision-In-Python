'''
LuvLinearScalingWindow.py
	
	Description:
		This program takes a color image and a window of the image as inputs, performs linear scaling
			in the Luv domain, and writes the scaled image as an output. It also displays the input
			image and the scaled output image.
'''
...
def linearScalingInLuvDomain(L,a,b):
	# L = L from Luv domain; a = smallest L in image; b = largest L in image
	if b != a: newL = ((L - a) * 100.0) / (b - a)
	else: newL = 0.0
	
	# Check bounds
	if newL < 0: newL = 0.0
	elif newL > 100: newL = 100.0
...
# Determine the a and b values used in linear scaling
smallestL = -1.0
largestL = -1.0
for i in range(int(H1), int(H2)+1) :
    for j in range(int(W1), int(W2)+1) :
        ...
        if (smallestL == -1.0) or (smallestL > L): smallestL = L
        if (largestL == -1.0) or (largestL < L): largestL = L

# Loop through the window and create the new scaled image
outputImage = np.copy(inputImage)
for i in range(int(H1), int(H2)+1) :
    for j in range(int(W1), int(W2)+1) :
        b, g, r = inputImage[i, j]
        L, u, v = convertBGR2Luv(b,g,r)
        newL = linearScalingInLuvDomain(L, smallestL, largestL)
        b,g,r = convertLuv2BGR(newL, u, v)
        outputImage[i, j] = [b,g,r]

cv2.imshow("p2:", outputImage)
