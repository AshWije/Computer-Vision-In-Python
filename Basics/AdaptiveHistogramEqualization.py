# AdaptiveHistogramEqualization.py
#
# Description:
#   This program gets as input a window parameter (w) and names of the input and the
#   output images. The output image is computed from the input image by changing only
#   the L values such that L(i, j) is computed by applying histogram equalization to
#   the window of size (2w+1 × 2w+1) centered at i, j. The L value at the center of the
#   window is the output value L(i, j). It should be executed as follows:
#       python AdaptiveHistogramEqualization.py <w> <inputImage> <outputImage>
...
for i in range(w, rows-w) :
    for j in range(w, cols-w) :
        ...
        # Perform Histogram Equalization on L and get value at pixel (i,j)
        midIndex = L_values.size // 2
        newL = cv2.equalizeHist(L_values)[midIndex]

        # Get L, u, and v values at pixel (i,j)
        _, u, v = luvImage[i, j]
        
        # Update L value at pixel (i,j)
